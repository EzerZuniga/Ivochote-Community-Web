...
const CATEGORY_STYLE: Record<string, { dateBg: string; badge: string; accent: string }> = {
  institucional: { dateBg: "bg-primary-700",  badge: "bg-primary-100 text-primary-800",   accent: "bg-primary-600"  },
  "econ\u00f3mico":  { dateBg: "bg-amber-700",    badge: "bg-amber-100 text-amber-800",     accent: "bg-amber-500"    },
  cultural:      { dateBg: "bg-rose-700",      badge: "bg-rose-100 text-rose-800",       accent: "bg-rose-500"     },
  ambiental:     { dateBg: "bg-emerald-700",   badge: "bg-emerald-100 text-emerald-800", accent: "bg-emerald-600"  },
  deportivo:     { dateBg: "bg-blue-700",      badge: "bg-blue-100 text-blue-800",       accent: "bg-blue-600"     },
};
  ...
  return (
    <article className="group flex flex-col bg-white border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200 h-full">
      ...
      <div className="flex gap-4 p-5 flex-1">
        {/* Bloque de fecha */}
        {eventDate && (
          <div className={`flex-shrink-0 w-14 flex flex-col items-center justify-center ${style.dateBg} text-white shadow-sm self-start`}>
            <span className="text-xl font-extrabold leading-none pt-1.5">{day}</span>
            <span className="text-[9px] font-bold uppercase tracking-widest pb-1.5">{month}</span>
            <div className="w-full text-center bg-black/20 text-[9px] font-semibold py-0.5">{year}</div>
          </div>
        )}
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            {status && (
              <span className={`text-[10px] font-bold px-2 py-0.5 ${status.cls}`}>
                {status.label}
              </span>
            )}
          </div>
          ...
          {href && (
            ...
            >
              Ver evento
              <svg className="w-3 h-3 transition-transform duration-150 group-hover:translate-x-0.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
